...
def iterative_broadcast(*args):
    ...
    if len(iterable_args) > 0:
        ...
        if (dimensions[huge_i] > min(ITERABLE_AMOUNT_OF_DIMS) and
            dimensions[huge_i] < max(ITERABLE_AMOUNT_OF_DIMS)):
...
def getexcentricity(gamma):
    ...
    if gpu.cuda_can_help:
        ...
        result = (1.000110 + 0.034221 * np.cos(gamma) + 0.001280 * np.sin(gamma)
...
def getbeamirradiance(extraterrestrialirradiance, excentricity, zenitangle,
	...
	#print (excentricity.shape, zenitangle.shape, solarelevation.shape,
    #       linketurbidity.shape, terrainheight.shape)
...
def getatmosphericradiance(extraterrestrialirradiance, i0met,diffuseclearsky,
                           satellitalzenitangle):
