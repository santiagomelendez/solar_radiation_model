...
class PreLaunch(object):
    ...
    def table(self):
        page = urllib2.urlopen("http://www.star.nesdis.noaa.gov/smcd/spb/fwu/"
                               "homepage/GOES_Imager_Vis_PreCal.php")
    ...
    def coefficients(self, table):
        co = {}
        for r in table[1:]:
            co[getsat(r[0].text_content())] = [getfloat(e.text_content())
                                               for e in r.getchildren()[1:]]
        return co
...
class PostLaunch(object):
    ...
    def satellites(self, table):
        sats = table[1:2]
        sats = [getsat(s.text_content()) for s in sats[0].getchildren()
    ...
    def coefficients(self, table):
        co = {}
        for r in table[2:]:
            co[getmonth(r[0].text_content())] = [getfloat(e.text_content())
                                                 for e in r.getchildren()[1:]]
        return co
