...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Stream', fields ['root_path']
        db.create_index('plumbing_stream', ['root_path'])

        # Adding unique constraint on 'Stream', fields ['root_path']
        db.create_unique('plumbing_stream', ['root_path'])

        # Deleting field 'Image.stream'
        db.delete_column('plumbing_image', 'stream_id')

        # Adding index on 'Image', fields ['localname']
        db.create_index('plumbing_image', ['localname'])

        # Adding unique constraint on 'Image', fields ['localname']
        db.create_unique('plumbing_image', ['localname'])


    def backwards(self, orm):
        # Removing unique constraint on 'Image', fields ['localname']
        db.delete_unique('plumbing_image', ['localname'])

        # Removing index on 'Image', fields ['localname']
        db.delete_index('plumbing_image', ['localname'])

        # Removing unique constraint on 'Stream', fields ['root_path']
        db.delete_unique('plumbing_stream', ['root_path'])

        # Removing index on 'Stream', fields ['root_path']
        db.delete_index('plumbing_stream', ['root_path'])
    ...
    models = {
        ...
        'plumbing.image': {
            ...
            'localname': ('django.db.models.fields.TextField', [], {'unique': 'True', 'db_index': 'True'}),
